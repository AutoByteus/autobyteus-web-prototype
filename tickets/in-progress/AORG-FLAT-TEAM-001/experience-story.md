...
| 3 | Org authoring | Add/edit/reorder a Handoff and When guidance | Explicit From, To, and ordered When update; invalid/stale endpoints block Save | Handoff meaning and ownership are truthful |
| 4 | Org launch | Open launch then select Team | Start changes from disabled to enabled; selected Team coordinator is named | No implicit entry or fallback |
| 5 | Runtime | Inspect root and configured scope | `Agent Org run` badge and Team placements are explicit | Root subject is truthful |
| 6 | Runtime/history | Expand Task Team and select child | Temporary indigo `Task:` row reveals child; focused execution explains runtime-only identity | Task lineage is not configured nesting |
| 7 | Runtime/history | Select standalone Team root | `Standalone Agent Team run` and coordinator-led lifecycle appear | Standalone history remains distinct |
