...
- Preserve the accepted AgentTeam detail hierarchy and interaction: bordered Back action; neutral header card with initials/category/summary pills; Description facts; Instructions; Handoffs; and member cards with a baseline-style `View ↗` action. Opening a member reuses the accepted Agent detail surface, including its Description and Instructions, and **Back to team** restores the same future-state Team detail context. Use the concise visible heading `Handoffs`; supporting copy explains that the routing rules remain with the Team. Replace nested-Team facts with Runs and keep the Team surface self-contained; do not add AgentOrg promotion or type/reuse chrome.
...
- AgentTeams and AgentOrgs share the accepted neutral surface and blue interactive language. Distinguish member kinds through familiar person/group icons and labels only where the user is choosing between those kinds; do not create a separate purple sub-brand for AgentOrgs.
...
| Team detail | Description and Instructions; Agent membership and coordinator; per-member `View ↗` to the accepted Agent detail with return-to-Team context; Handoffs; standalone run/history; no AgentOrg cross-promotion |
| Org catalog | Selected `Agent Orgs` navigation plus screen-reader heading; baseline search/Reload/Create toolbar, initials/category cards, direct Agent/Team chips, Agents/Teams/Handoffs/Runs metrics, and baseline Run/View Details actions; no repeated type/composition/coordinator explanation |
| Org create/edit | Baseline-native Basics, Members, and Handoffs cards; separate Add Agent and Add Team actions; no coordinator control or explanatory coordinator panel |
| Org detail | Baseline-native header, Description, Members, and Handoffs cards; direct Agent and Team groups; same-definition/history statements; Run and Edit actions |
| Org launch | Modal opens with no entry and disabled Start; concise Agent/Team choice; selected Team states its entry coordinator; no implementation-oriented coordinator/fallback disclaimer |
...
| Open Team member | Baseline-style `View ↗` opens accepted Agent detail | Selected member Description and Instructions remain inspectable |
| Return from Agent detail | **Back to team** | Same future-state Team detail and prototype-review context are restored |
...
- Use structural absence rather than repeated `No coordinator` labels on Org catalog/detail/authoring. The launch selector names the exact Agent or Team choice; a Team choice names the coordinator used for entry. Runtime/history may use `Through coordinator`, `Configured Team placement`, and `Temporary execution` where those distinctions are contextually necessary.
...
| --- | --- | --- | --- |
| `REV-AORG-001` | Baseline-native Team catalog | 1440×900 | `review-evidence/rv-006/REV-AORG-001-agent-team-catalog.png` |
| `REV-AORG-002` | Baseline-native Team authoring | 1440×900 | `review-evidence/rv-006/REV-AORG-002-agent-team-authoring.png` |
| `REV-AORG-003` | Org catalog | 1440×900 | `review-evidence/rv-006/REV-AORG-003-agent-org-catalog.png` |
| `REV-AORG-004` | Org authoring | 1440×900 | `review-evidence/rv-006/REV-AORG-004-agent-org-authoring.png` |
| `REV-AORG-005` | Org detail | 1440×900 | `review-evidence/rv-006/REV-AORG-005-agent-org-detail.png` |
| `REV-AORG-006` | Exact Team entry selected | 1440×900 | `review-evidence/rv-006/REV-AORG-006-agent-org-exact-entry-launch.png` |
| `REV-AORG-007` | Org runtime and task child focus | 1440×900 | `review-evidence/rv-006/REV-AORG-007-agent-org-runtime-history.png` |
| `REV-AORG-008` | Standalone Team runtime | 1440×900 | `review-evidence/rv-006/REV-AORG-008-standalone-team-runtime-history.png` |
| `REV-AORG-009` | Baseline-native Team detail with concise Handoffs label | 1440×900 | `review-evidence/rv-006/REV-AORG-009-agent-team-detail.png` |
| `REV-AORG-010` | Preserved Team member cards and Agent-detail actions | 1440×900 | `review-evidence/rv-006/REV-AORG-010-agent-team-members.png` |
| `REV-AORG-011` | Accepted Agent detail reached from a Team member | 1440×900 | `review-evidence/rv-006/REV-AORG-011-agent-member-detail.png` |
...
- User approval or requested revision of the baseline-native, self-contained AgentTeam and AgentOrg surfaces.
...
- RV-006 browser validation: `30/30 pass`, zero runtime errors
