...
const numericFields = [
  'grossInputTokens', 'standardInputTokens', 'cacheReadInputTokens', 'cacheCreationInputTokens', 'outputTokens',
  'reasoningOutputTokens', 'totalTokens', 'estimatedApiTotalCost',
];
const displayRows = computed(() => {
  const groups = new Map<string, any>();
  for (const sourceRow of props.result.breakdownRows) {
    const key = identity(sourceRow);
    if (!groups.has(key.key)) {
      groups.set(key.key, { ...key, aggregate: { ...sourceRow.aggregate }, costQuality: sourceRow.costQuality, sourceRows: [sourceRow] });
      continue;
    }
    const group = groups.get(key.key);
    group.sourceRows.push(sourceRow);
    for (const field of numericFields) {
      const value = sourceRow.aggregate[field];
      if (value != null) group.aggregate[field] = (group.aggregate[field] ?? 0) + value;
    }
    if (group.costQuality.currency !== sourceRow.costQuality.currency) group.costQuality = { kind: 'MIXED_CURRENCY', currency: null, missingPriceDimensions: [] };
  }
  return [...groups.values()].sort((left, right) => right.aggregate.totalTokens - left.aggregate.totalTokens || left.label.localeCompare(right.label));
});
const qualityLabel = (kind: string) => t(`settings.components.settings.TokenUsageAnalytics.quality${kind}`);
const rowCost = (row: any) => formatTokenUsageAnalyticsCost({
...
const rowShare = (row: any) => {
  ...
  if (rowValue == null || totalValue == null || totalValue === 0 || (props.metric === 'COST' && row.costQuality.currency !== props.result.selectedCostQuality.currency)) return notAvailable.value;
  return percent(rowValue / totalValue);
};
const exactDetails = (row: any) => [
