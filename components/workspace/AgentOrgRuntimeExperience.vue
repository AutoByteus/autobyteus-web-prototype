    ...
    <div class="mx-auto w-full max-w-[1500px] px-5 py-6 sm:px-7 lg:px-9">
      <header class="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
        ...
        <div class="mt-5 grid gap-3 md:grid-cols-3">
          ...
          <div class="rounded-xl border border-slate-200 bg-slate-50 p-4"><p class="text-xs font-semibold uppercase tracking-wide text-slate-500">Execution lineage</p><p class="mt-2 text-sm font-bold text-slate-900">1 Task Agent · 1 Task Team</p><p class="mt-1 text-xs text-slate-500">Temporary rows under the owning Team run</p></div>
        </div>
      </header>
      ...
      <div class="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1.15fr)_minmax(22rem,0.85fr)]">
        <div class="space-y-5">
          ...
          <section class="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" data-test="runtime-execution-lineage">
            <div class="flex flex-col gap-3 border-b border-slate-100 px-5 py-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <div class="flex flex-wrap items-center gap-2">
                  <h2 class="font-bold text-slate-950">Runtime execution lineage</h2>
                  <span class="rounded bg-indigo-50 px-2 py-0.5 text-[0.625rem] font-bold uppercase tracking-wide text-indigo-700">Temporary executions</span>
                </div>
                <p class="mt-1 text-sm text-slate-500">Task rows remain inside the owning Team run. They never become AgentOrg or AgentTeam definition members.</p>
              </div>
              <span class="inline-flex items-center gap-1.5 self-start rounded-full bg-slate-100 px-2.5 py-1 text-xs font-semibold text-slate-600"><Icon icon="heroicons:eye-20-solid" class="h-3.5 w-3.5" /> Runtime view</span>
            </div>

            <div class="p-5">
              <div class="rounded-xl border border-slate-200 bg-slate-50/70 p-3" data-test="runtime-owning-team">
                <div class="flex items-center gap-2 rounded-lg bg-white px-3 py-2.5 ring-1 ring-slate-200">
                  <Icon icon="heroicons:chevron-down-20-solid" class="h-4 w-4 text-slate-400" />
                  <Icon icon="heroicons:user-group-20-solid" class="h-4 w-4 text-blue-600" />
                  <span class="min-w-0 flex-1 truncate text-sm font-semibold text-slate-900">Product Design & Prototyping</span>
                  <span class="rounded bg-blue-50 px-1.5 py-0.5 text-[0.625rem] font-bold uppercase text-blue-600">Configured Team placement</span>
                </div>

                <div class="ml-5 mt-2 border-l border-slate-300 pl-3">
                  <div class="flex items-center gap-2 rounded-lg px-3 py-2 text-sm text-slate-700">
                    <span class="h-2 w-2 rounded-full bg-emerald-500"></span>
                    <span class="inline-flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-[0.625rem] font-bold text-slate-600">PP</span>
                    <span class="min-w-0 flex-1 truncate font-medium">product_prototyper</span>
                    <span class="text-[0.625rem] font-semibold uppercase text-slate-400">Configured Agent</span>
                  </div>

                  <button
                    type="button"
                    data-test="runtime-task-agent-row"
                    class="ml-5 mt-1 flex w-[calc(100%_-_1.25rem)] items-center gap-2 rounded-lg bg-indigo-50/70 px-3 py-2 text-left text-sm text-indigo-950 ring-1 transition hover:bg-indigo-50"
                    :class="taskAgentSelected ? 'ring-indigo-300' : 'ring-indigo-100'"
                    @click="selectTaskFocus('task-agent-launch-audit')"
                  >
                    <Icon icon="svg-spinners:ring-resize" class="h-4 w-4 flex-none text-indigo-600" />
                    <span class="min-w-0 flex-1 truncate">Task: Audit launch evidence</span>
                    <span class="text-[0.625rem] font-bold uppercase text-indigo-600">Task Agent</span>
                  </button>

                  <button
                    type="button"
                    data-test="runtime-task-team-row"
                    class="mt-2 flex w-full items-center gap-2 rounded-lg bg-indigo-50/70 px-3 py-2 text-left text-sm text-indigo-950 ring-1 ring-indigo-100"
                    aria-expanded="true"
                    @click="selectTaskFocus('task-team-accessibility:reviewer')"
                  >
                    <Icon icon="heroicons:chevron-down-20-solid" class="h-4 w-4 flex-none text-indigo-400" />
                    <Icon icon="svg-spinners:ring-resize" class="h-4 w-4 flex-none text-indigo-600" />
                    <span class="min-w-0 flex-1 truncate">Task: Accessibility evidence review</span>
                    <span class="text-[0.625rem] font-bold uppercase text-indigo-600">Task Team</span>
                  </button>

                  <button
                    type="button"
                    data-test="runtime-task-team-child-row"
                    class="ml-7 mt-1 flex w-[calc(100%_-_1.75rem)] items-center gap-2 rounded-lg px-3 py-2 text-left text-sm transition"
                    :class="taskChildSelected ? 'bg-indigo-50 text-indigo-950 ring-1 ring-indigo-200' : 'text-slate-600 hover:bg-slate-50'"
                    @click="selectTaskFocus('task-team-accessibility:reviewer')"
                  >
                    <Icon icon="svg-spinners:ring-resize" class="h-4 w-4 flex-none text-indigo-600" />
                    <span class="min-w-0 flex-1 truncate">reviewer</span>
                    <span class="text-[0.625rem] font-semibold uppercase text-slate-400">Temporary child AgentRun</span>
                  </button>
                </div>
              </div>

              <div class="mt-3 flex items-start gap-2 rounded-lg border border-indigo-100 bg-indigo-50/60 px-3 py-2.5 text-xs leading-5 text-indigo-900">
                <Icon icon="heroicons:information-circle-20-solid" class="mt-0.5 h-4 w-4 flex-none" />
                <p><strong>Definition remains flat.</strong> The indigo <span class="font-mono">Task:</span> rows are temporary execution identities following the source-current Workspace pattern, not authored nesting.</p>
              </div>
            </div>
          </section>
        </div>
        ...
        <aside class="space-y-5">
          <section v-if="selectedTaskFocus" class="rounded-2xl border border-indigo-200 bg-indigo-50/70 p-5 shadow-sm" data-test="selected-runtime-execution">
            <div class="flex items-start gap-3">
              <span class="inline-flex h-9 w-9 flex-none items-center justify-center rounded-lg bg-white text-indigo-700 ring-1 ring-indigo-200"><Icon icon="svg-spinners:ring-resize" class="h-5 w-5" /></span>
              <div class="min-w-0">
                <p class="text-xs font-semibold uppercase tracking-wide text-indigo-600">Selected temporary execution</p>
                <h2 class="mt-1 truncate font-bold text-indigo-950">{{ selectedExecutionTitle }}</h2>
                <p class="mt-1 text-sm leading-6 text-indigo-900/80">{{ selectedExecutionDetail }}</p>
              </div>
            </div>
          </section>
...
const selectedTaskFocus = computed(() => typeof route.query.focus === 'string' ? route.query.focus : '');
const taskAgentSelected = computed(() => selectedTaskFocus.value === 'task-agent-launch-audit');
const taskChildSelected = computed(() => selectedTaskFocus.value === 'task-team-accessibility:reviewer');
const selectedExecutionTitle = computed(() => taskAgentSelected.value
  ? 'Task: Audit launch evidence'
  : 'reviewer · Accessibility evidence review');
const selectedExecutionDetail = computed(() => taskAgentSelected.value
  ? 'Temporary Task Agent under product_prototyper. Its work and conversation belong to this run only.'
  : 'Temporary child AgentRun inside the Task Team execution. It is not a configured AgentOrg or AgentTeam member.');
