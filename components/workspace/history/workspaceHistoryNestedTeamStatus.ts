...
const normalizedAgentStatus = (status: AgentStatus | string | null | undefined): AgentStatus => {
...
const rowAgentStatus = (row: RunHistoryTeamExecutionRow): AgentStatus => {
  ...
  return normalizedAgentStatus(row.kind === 'stable_member' ? row.row.currentStatus : row.currentStatus);
};
