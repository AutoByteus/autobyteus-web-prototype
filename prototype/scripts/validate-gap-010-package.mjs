...
const currentRoot = '/home/autobyteus/workspace/autobyteus-web-prototype'
const staleRoots = [
...
check('Validation runs on the independent personal branch', git('branch', '--show-current') === 'personal')
let importedSourceHistory = false
try { gitRaw('cat-file', '-e', `${sourcePin}^{commit}`); importedSourceHistory = true } catch {
  // Expected in the clean independent history.
}
check('Approved source pin is documented without importing source-repository history', allText.includes(sourcePin) && !importedSourceHistory)
const changedPaths = gitRaw('status', '--porcelain').trimEnd().split('\n').filter(Boolean).map(line => line.slice(3))
check('Working tree is clean or changes are isolated to independent prototype content', changedPaths.every(path => !path.startsWith('../') && path !== '.gitmodules'), changedPaths.length ? `${changedPaths.length} prototype path(s)` : 'clean')
let selectedSourceCopied = true
try { await access(resolve(root, 'autobyteus-web')) } catch { selectedSourceCopied = false }
