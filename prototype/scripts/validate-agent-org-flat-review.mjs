...
const evidenceRoot = path.join(ticketRoot, 'review-evidence', 'rv-001');
...
record('AORG-RV1-001-team-catalog',
...
record('AORG-RV1-002-separated-primary-navigation',
...
record('AORG-RV1-003-team-card-flat-contract',
...
record('AORG-RV1-004-team-authoring-agent-only',
...
record('AORG-RV1-005-exactly-one-team-coordinator',
...
record('AORG-RV1-006-org-catalog',
...
record('AORG-RV1-007-org-direct-composition',
...
record('AORG-RV1-008-org-authoring-membership',
...
record('AORG-RV1-009-org-has-no-coordinator-control',
  await orgCreate.locator('input[type="radio"]').count() === 0
    && await orgCreate.getByText('No Organization coordinator', { exact: true }).count() === 1,
...
record('AORG-RV1-010-team-reference-preservation',
...
record('AORG-RV1-011-org-launch-requires-entry',
...
record('AORG-RV1-012-team-entry-resolves-through-coordinator',
...
record('AORG-RV1-013-launch-carries-exact-entry',
  ...
  { url: orgDetail.url() });
record('AORG-RV1-014-runtime-root-kinds',
  await orgDetail.locator('[data-test="history-org-root"]').count() === 1
    && await orgDetail.locator('[data-test="history-team-root"]').count() === 1,
  { roots: ['Org', 'Team'] });
record('AORG-RV1-015-task-team-is-runtime-lineage',
  await orgDetail.getByText('Task work', { exact: true }).count() >= 2
    && await orgDetail.getByText('Runtime lineage', { exact: true }).count() >= 1
    && await orgDetail.getByText('not configured children of the Org or Team definition', { exact: false }).count() === 1,
  { taskPresentation: 'separate runtime lineage' });
...
record('AORG-RV1-016-standalone-team-history-remains-distinct',
...
record('AORG-RV1-017-narrow-layout', overflow.scrollWidth === overflow.viewport
  && await narrow.getByRole('heading', { name: 'Software Development Department' }).count() === 1,
{ overflow });
await narrow.close();

record('AORG-RV1-018-no-runtime-errors', runtimeErrors.length === 0, { runtimeErrors });
...
await fs.writeFile(path.join(ticketRoot, 'browser-validation-rv-001.json'), `${JSON.stringify(result, null, 2)}\n`);
