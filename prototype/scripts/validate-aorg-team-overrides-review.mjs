...
const evidenceRoot = path.resolve(process.env.AORG_TEAM_OVERRIDES_EVIDENCE_ROOT || path.join(ticketRoot, 'review-evidence', 'rv-003'));
...
const disclosureStyle = async locator => locator.evaluate((node) => {
...
const orgDisclosureStyle = await disclosureStyle(initialToggle);
...
record('AORG-OVERRIDES-003-exact-agent-member-count',
  (await initialPage.locator('[data-test="org-member-overrides-label"]').innerText()) === 'Member overrides (7)',
  { label: await initialPage.locator('[data-test="org-member-overrides-label"]').innerText() });
await initialPage.screenshot({ path: path.join(evidenceRoot, 'RV3-001-initial-outer-collapsed-1440x900.png') });
...
record('AORG-OVERRIDES-004-all-mounted-teams-collapsed',
...
record('AORG-OVERRIDES-005-direct-agent-preserved',
...
await collapsedPage.screenshot({ path: path.join(evidenceRoot, 'RV3-002-outer-open-teams-collapsed-1440x900.png') });
...
record('AORG-OVERRIDES-006-one-team-expanded-inherited',
...
record('AORG-OVERRIDES-007-coordinator-secondary-identity',
...
await expandedPage.screenshot({ path: path.join(evidenceRoot, 'RV3-003-team-expanded-inherited-1440x900.png') });
...
record('AORG-OVERRIDES-008-team-and-agent-scope-local-customization',
...
record('AORG-OVERRIDES-009-collapse-reopen-preserves-valid-draft',
...
await firstHeader.scrollIntoViewIfNeeded();
await expandedPage.screenshot({ path: path.join(evidenceRoot, 'RV3-004-team-agent-customized-reopened-1440x900.png') });
...
record('AORG-OVERRIDES-010-agent-only-does-not-customize-team',
...
record('AORG-OVERRIDES-011-responsive-no-overlay-or-horizontal-overflow',
...
await narrowPage.screenshot({ path: path.join(evidenceRoot, 'RV3-005-outer-open-teams-collapsed-narrow-390x844.png') });
...
const teamDisclosureStyle = await disclosureStyle(teamMemberToggle);
record('AORG-OVERRIDES-012-agentteam-launch-baseline-preserved',
...
record('AORG-OVERRIDES-013-disclosure-style-matches-agentteam',
  JSON.stringify(orgDisclosureStyle) === JSON.stringify(teamDisclosureStyle),
  { orgDisclosureStyle, teamDisclosureStyle });
await teamPage.close();

await browser.close();
record('AORG-OVERRIDES-014-zero-browser-errors', runtimeErrors.length === 0, { runtimeErrors });
