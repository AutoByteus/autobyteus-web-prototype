...
const evidenceRoot = path.resolve(process.env.AORG_TEAM_OVERRIDES_EVIDENCE_ROOT || path.join(ticketRoot, 'review-evidence', 'rv-001'));
...
await initialPage.screenshot({ path: path.join(evidenceRoot, 'RV-001-initial-outer-collapsed-1440x900.png') });
...
record('AORG-OVERRIDES-002-all-mounted-teams-collapsed',
...
record('AORG-OVERRIDES-003-direct-agent-preserved',
...
await collapsedPage.screenshot({ path: path.join(evidenceRoot, 'RV-002-outer-open-teams-collapsed-1440x900.png') });
...
record('AORG-OVERRIDES-004-one-team-expanded-inherited',
...
record('AORG-OVERRIDES-005-coordinator-secondary-identity',
...
await expandedPage.screenshot({ path: path.join(evidenceRoot, 'RV-003-team-expanded-inherited-1440x900.png') });
...
record('AORG-OVERRIDES-006-team-and-agent-scope-local-customization',
...
record('AORG-OVERRIDES-007-collapse-reopen-preserves-valid-draft',
...
await firstHeader.scrollIntoViewIfNeeded();
await expandedPage.screenshot({ path: path.join(evidenceRoot, 'RV-004-team-agent-customized-reopened-1440x900.png') });
...
record('AORG-OVERRIDES-008-agent-only-does-not-customize-team',
...
record('AORG-OVERRIDES-009-responsive-no-overlay-or-horizontal-overflow',
...
await narrowPage.screenshot({ path: path.join(evidenceRoot, 'RV-005-outer-open-teams-collapsed-narrow-390x844.png') });
...
record('AORG-OVERRIDES-010-agentteam-launch-baseline-preserved',
...
await browser.close();
record('AORG-OVERRIDES-011-zero-browser-errors', runtimeErrors.length === 0, { runtimeErrors });
